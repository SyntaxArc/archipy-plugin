...
3. Read `reference.md` in this skill directory (version, install, layout, and a topic index), then only the matching
   `reference/<topic>.md`; it targets ArchiPy 5.x. For older apps, explain migration differences
...
| Health checks     | Bundled `reference/health-checks.md` (HTTP + gRPC) + `/scaffold-health-checks`                                      |
...
| Observability     | Bundled `reference/observability.md` + https://syntaxarc.github.io/ArchiPy/tutorials/observability/ |
| Redis search      | Bundled `reference/adapters.md` + `/redis-search`                                                              |
