...
Read and follow the **scaffold-archipy-health-checks** skill in full. Inspect the workspace as directed there; ask only
for unresolved choices.

1. Generate shared check helpers plus:
...
2. Wire via `AppUtils.create_fastapi_app()` / `create_grpc_app` / `create_async_grpc_app`; emit `deploy/k8s-probes.yaml`
